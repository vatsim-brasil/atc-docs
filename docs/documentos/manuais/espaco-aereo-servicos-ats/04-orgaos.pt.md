...
Duas advertências sobre a coluna "Posição na rede". A primeira: os sufixos `_CTR`, `_APP`, `_TWR`, `_GND` e `_DEL` são convenção de **toda a rede VATSIM**, como a seção anterior registrou — não invenção brasileira. O que é próprio da **Vatsim Brasil** nesta tabela são o `_R_TWR` das posições RÁDIO, cujo marcador `_R_` é daqui, e a ausência de posição para o Centro de Informação de Voo. A segunda: "CTR" carrega dois sentidos neste manual — o sufixo `_CTR`, que é a posição do ACC, e a Zona de Controle, que é espaço aéreo. Na coluna de espaço aéreo a Zona de Controle está escrita por extenso, para que as duas não se confundam.

| Órgão ATS | Posição interna | Posição na rede | Espaço aéreo típico | Serviço prestado |
| --- | --- | --- | --- | --- |
| Centro de Controle de Área (ACC) | — | `_CTR` | CTA, UTA e demais porções da FIR | Controle de área |
| Controle de Aproximação (APP) | — | `_APP` | TMA e Zona de Controle\* | Controle de aproximação |
| Torre de Controle de Aeródromo (TWR) | Torre de Controle | `_TWR` | ATZ e Zona de Controle\* | Controle de aeródromo |
